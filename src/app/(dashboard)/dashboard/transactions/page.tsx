...
export default async function TransactionsPage() {
    ...
    return (
        <div className="min-h-screen bg-background text-foreground p-6 lg:p-8 font-sans">
                ...
                {transactions && transactions.length > 0 ? (
                    <div className="overflow-x-auto">
                        <table className="w-full text-left text-sm text-muted-foreground">
                            <thead className="bg-secondary/40 text-foreground uppercase tracking-wider text-xs font-semibold border-b border-border">
                                <tr>
                            ...
                            <tbody className="divide-y divide-border">
                                {transactions.map((t: any) => (
                                    <tr key={t._id} className="group hover:bg-secondary/20 transition-colors duration-200">
                                        ...
                                        <td className="px-6 py-4 text-muted-foreground max-w-xs truncate font-mono text-xs" title={t.description}>
