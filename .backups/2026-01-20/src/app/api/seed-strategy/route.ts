...
export async function GET() {
    try {
        ...
        const newStrategy = await InvestmentStrategy.create({
            ...
            minInvestment: 5000,
            lockInPeriod: 12,
            estimatedReturn: "18-24%",
            allocation: [
                { asset: "Technology Growth Stocks", percentage: 35, color: "#6366f1" }, // Indigo
                { asset: "Commercial Real Estate", percentage: 30, color: "#10b981" }, // Emerald
                { asset: "Green Energy Bonds", percentage: 15, color: "#f59e0b" }, // Amber
                { asset: "Digital Assets (Blue Chip)", percentage: 10, color: "#8b5cf6" }, // Violet
                { asset: "Liquid Reserves", percentage: 10, color: "#3b82f6" }, // Blue
            ],
